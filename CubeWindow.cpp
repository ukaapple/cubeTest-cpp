...
CubeWindow::CubeWindow(int xBlocks, int yBlocks, int zBlocks) {
    _xBlocks = xBlocks;
    _yBlocks = yBlocks;
    _zBlocks = zBlocks;
}
...
int CubeWindow::run() {
    ...
    // create window
    GLFWwindow *const window(glfwCreateWindow(_WIN_W, _WIN_H, "CubeWindow", NULL, NULL));
    if (NULL == window) {
    ...
    _programID = loadShaders("shader.vert", "shader.frag");
    ...
    this->makeData(xs, ys, zs, xm, ym, zm,
                    ...
                    &_vertAry, &_texAry, &_idxAry,
                    &vertAryNum, &texAryNum, &idxAryNum);
    cout << &_vertAry << endl;
    ...
    while (glfwWindowShouldClose(window) == GL_FALSE) {
        ...
        glDrawElements(GL_TRIANGLE_STRIP, idxAryNum - 1, GL_UNSIGNED_INT, 0);
...
GLuint CubeWindow::loadShaders(const char * vertexPath,const char * fragmentPath) {
...
void CubeWindow::getShaderInfoLog(GLuint id, int length) {
...
void CubeWindow::getProgramInfoLog(GLuint id, int length) {
    GLchar *infoLog = (GLchar *)malloc(length);
    if (NULL != infoLog) {
...
int CubeWindow::loadTexture(string fileName, GLenum texUnit, int width, int height) {
...
void CubeWindow::makeData(int xs, int ys, int zs, int xm, int ym, int zm, float xd, float yd, float zd, 
