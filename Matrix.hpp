...
struct Matrix4 {
    ...
    static Matrix4<T> Rotation(T radians, T x, T y, T z)
    {
        ...
        Matrix4 m;
        m.x.x = cos + cosp * v.x * v.x;
        m.x.y = cosp * v.x * v.y + v.z * sin;
        m.x.z = cosp * v.x * v.z - v.y * sin;
        m.x.w = 0.0f;
        m.y.x = cosp * v.x * v.y - v.z * sin;
        m.y.y = cos + cosp * v.y * v.y;
        m.y.z = cosp * v.y * v.z + v.x * sin;
        m.y.w = 0.0f;
        m.z.x = cosp * v.x * v.z + v.y * sin;
        m.z.y = cosp * v.y * v.z - v.x * sin;
        m.z.z = cos + cosp * v.z * v.z;
        m.z.w = 0.0f;
        m.w.x = 0.0f;
        m.w.y = 0.0f;
        m.w.z = 0.0f;
    ...
    static Matrix4<T> Perspective(T degrees, T aspectRatio, T near, T far)
    {
        T top = near * std::tan(degrees * M_PI / 360.0f);
        T bottom = -top;
        T left = bottom * aspectRatio;
